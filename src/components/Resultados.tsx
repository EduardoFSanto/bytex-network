...
const cases = [
  {
    ...
    resultado: "+480% em vendas online em 3 meses",
    tag: "Meta Ads",
  },
  {
    empresa: "Clínica Odontológica",
    servico: "Redes Sociais",
    resultado: "De 800 para 28K seguidores em 6 meses",
    tag: "Instagram",
  },
  {
    empresa: "Imobiliária Regional",
    servico: "Site + Google Ads",
    resultado: "12x mais leads qualificados por mês",
    tag: "Google Ads",
  },
];
...
export default function Resultados() {
  return (
    <section id="resultados" className="py-24 relative">
      <div className="absolute top-1/3 right-0 w-96 h-96 bg-purple-700/10 rounded-full blur-[120px]" />
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
          {numeros.map(({ value, label }) => (
            <div key={label} className="card-dark rounded-2xl p-6 text-center">
              <p className="gradient-text text-3xl md:text-4xl font-black mb-2">
                {value}
              </p>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {cases.map(({ empresa, servico, resultado, tag }) => (
            <div
              key={empresa}
              className="card-dark rounded-2xl p-6 group hover:-translate-y-1 transition-all duration-300"
            >
              <div className="flex items-center justify-between mb-4">
                <span className="text-xs font-medium bg-purple-500/20 text-purple-300 px-3 py-1 rounded-full">
                  {tag}
                </span>
                <span className="text-white/30 text-xs">{servico}</span>
              </div>
              <h4 className="text-white font-bold text-base mb-2">{empresa}</h4>
              <p className="gradient-text font-black text-lg leading-tight">
                {resultado}
              </p>
            </div>
          ))}
